...
class PopupIndex4 extends Component {
  ...
  handleSubmit = event => {
    ...
    if (this.validateForm()) {
      ...
      if(patient_id){
        this.LoginPaitentId(patient_id);
      } else {
        fields["phone_number"] = "";
    ...
    .then(res => {
      if (!!res[0].token) {
        ...
        .then(patient_details_res => {
          ...
          let search_selected_doc_name = '';
          let doctor_profile_avability = '';
          startUserSession(res[0].token, patient_id, login_phone_number, search_selected_doc_id, schedule_date, apointment_id, doctor_name, doctor_fees, patientName, schedule_time, search_selected_doc_name, doctor_profile_avability);
              ...
              <form onSubmit={this.handleSubmit} id="loginform" className="needs-validation" noValidate>
                ...
                <div className="row mt-3">
                  <div className="col-12">
                    <button type="submit" className="btn-links appointment-new" id="black-btn" disabled> Login </button>
                  </div> 
                </div> 
